...
| 更新版本后无需重新适配 | ✅                                  | ❌                        | ⭕ 取决于导出符号与 V8 ABI                        |
| 允许修改任意脚本    | ❌                                  | ✅ 支持对原脚本替换               | ✅ 支持对原脚本替换或**修改**                         |

该方案在内存中修改编译输入。尚未验证真实 Electron 注入；仅完成符号/ABI 适配不代表已支持。
...
CI 已配置 Node 22/24/26 的 Linux x64 与 Windows GNU/Wine 覆盖，尚未远程运行。Windows 覆盖不代表原生 Windows/MSVC 已验证。macOS 为实验性支持，没有 CI 测试。Electron 注入仍未验证，CEF 未测试，Deno 不支持。

V8 Killer 的缺点主要集中在以下几点：

- 兼容性取决于具体构建的导出符号与 V8 ABI；缺少必要导出的构建需要显式配置函数 RVA，并具备匹配的 ABI 支持；
