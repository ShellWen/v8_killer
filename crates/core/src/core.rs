...
use crate::v8_sys::{local_string_from_string, string_from_local_string, v8_context_get_isolate, V8Context, V8Source};

pub(crate) unsafe fn process_script(config: &Config, v8_context: *const V8Context, v8_source: *mut V8Source) {
    ...
    config.rules.iter().for_each(|rule_item| {
        ...
        if *is_match {
            println!("[*] Rule {} matched in {}", rule_name, &source.resource_name);
            ...
            processors.iter().for_each(|processor_item| {
                ...
                if result.is_err() {
                    println!("[!] Processor {:#?} process failed: {}", processor, result.err().unwrap());
                };
            });
        }
    });
    let processed_source_string = source.source_string.as_str();
    let processed_local_string = local_string_from_string(isolate, processed_source_string.to_string());
